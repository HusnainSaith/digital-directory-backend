...
export class SecurityUtil {
  ...
  static sanitizeString(input: string): string {
    ...
    return (
      input
        .replace(/[<>]/g, '') // Remove HTML/angle bracket chars to prevent XSS
        .replace(/[{}]/g, '') // Remove braces
        .replace(/[\\[\\]]/g, '') // Remove brackets
        .replace(/\\$/g, '') // Remove dollar signs (MongoDB operators)
        // Note: quotes are intentionally NOT removed — TypeORM uses parameterized queries
        // which already prevent SQL injection. Removing quotes corrupts valid data
        // (names like O'Brien, descriptions using "double quotes", etc.)
        .trim()
    );
  }
  ...
  static sanitizeInput(input: string): string {
    ...
    return input
      .trim()
      .replace(/[<>%;()&+]/g, '') // Remove potentially dangerous characters (quotes kept for valid data)
